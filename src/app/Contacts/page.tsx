...
export default function Contacts() {
  return (
    <section className=" flex flex-col  justify-center items-center bg-orange-300 md:p-8 lg:p-8 p-2 min-h-screen  w-full   ">
      ...
      <nav className="bg-slate-300 shadow-lg md:w-[43%] lg:w-[43%] w-full   rounded-t-xl ">
        <ul className=" flex flex-wrap gap-2  items-center p-4  justify-between">
          <li className="cursor-pointer  md:text-xl max-lg:xl text-sm  hover:border-red-500  hover:border-b-4  ">
            {" "}
            Provider
          </li>
          <li className="cursor-pointer md:text-xl max-lg:xl text-sm   hover:border-red-500  hover:border-b-4 ">
      ...
      <section className="border-2 shadow-xl md:w-[43%] lg:w-[43%] w-full rounded-b-xl">
        <ul className="p-2 space-y-1 bg-gray-100 rounded-md  md:max-h-[450px] lg:max-h-[450px]  max-h-[565px]  overflow-y-scroll">
          {/* <PerfectScrollbar style={{ maxHeight: '15rem' }}> */}
          {MockContacts.map((e) => (
            <li
              key={e.id}
              className="border-2 rounded-lg cursor-pointer bg-white scroll-smooth p-3 md:text-xl max-lg:xl text-sm font-bold"
            >
